...
class ModuleUtil extends Util
{
     ...
     * @return boolean
     */
    public function isProductInstalled($module_name)
    {
      
      $product = SuperadminProduct::where('name',$module_name)->where('code',201)->first() ;  
     if ($product){
      $is_available = SuperadminBusinessProduct::where('product_id',$product->id)->first();
      // dd($is_available);
        if ($is_available) {
            //Check if installed by checking the system table {module_name}_version
            $module_version = System::getProperty(strtolower($module_name) . '_version');
    ...
    public function getModuleData($function_name, $arguments = null)
    {
        ...
        // foreach ($modules as $module => $details) {
        //   if ($this->isProductInstalled($details['name'])) 
        //         $installed_modules[] = $details;
        //     }
           

        // dd($installed_products);
        $data = [];
    ...
    public function hasProduct( $code )
    {
        if ($this->isSuperadminInstalled()) {
            ...
            $is_available = SuperadminBusinessProduct::where('business_id' , session('business.id'))->where('product_code',$code)->latest()->first();
            $date = date('Y-m-d');
            
           if ($is_available){
            if($is_available->end_date != null && $date > $is_available->end_date){
              return false;
            }
            return true;
          }else{
    ...
    {
        $count = BusinessLocation::where('business_id', $business_id)->where('is_warehouse', '0')->where('is_sup_warehouse', '0')
                                ->count();

        return $count;
    }
    public function countWarehouse($business_id)
    {
        $count = BusinessLocation::where('business_id', $business_id)->where('is_warehouse' ,1)
                                ->count();

        return $count;
    }
    public function countSupWarehouse($business_id)
    {
        $count = BusinessLocation::where('business_id', $business_id)->where('is_sup_warehouse' ,1)
                                ->count();
    ...
    public function countUsers($business_id)
    {
        $count = User::where('business_id', $business_id)
                                    ->where('allow_login', 1)->where('is_employee', 0)
                                    ->count();
    ...
    public function isQuotaAvailable($type, $business_id, $total_rows = 0)
    {
        ...
        if ($is_available) {
            ...
            if ($type == 'locations') {
              ...
              $max_allowed = isset($package->package_details['warehouse_count']) ? $package->package_details['warehouse_count'] : 0;
              if ($max_allowed == 0 ) {
                  return true;
              } else {
                  $count = $this->countWarehouse($business_id);
                  if ($count >= $max_allowed) {
                      return false;
                  }
              }
            } elseif($type == 'sup_warehouse'){
              $max_allowed = isset($package->package_details['sup_warehouse']) ? $package->package_details['sup_warehouse'] : 0;
              if ($max_allowed == 0 ) {
                  return true;
              } else {
                  $count = $this->countSupWarehouse($business_id);
                  if ($count >= $max_allowed) {
                      return false;
                  }
              }
    ...
    public function quotaExpiredResponse($type, $business_id, $redirect_url = null)
    {
        if ($type == 'locations') {
          ...
          if (request()->ajax()) {
            if (request()->wantsJson()) {
                $response_array = ['success' => 0,
                        'msg' => __("superadmin::lang.max_warehouse")
                    ];
                return $response_array;
            } else {
                return view('superadmin::subscription.max_warehouse_modal');
            }
        }
        }  elseif ($type == 'sup_warehouse') {
          if (request()->ajax()) {
            if (request()->wantsJson()) {
                $response_array = ['success' => 0,
                        'msg' => __("superadmin::lang.max_sup_warehouse_count")
                    ];
                return $response_array;
            } else {
                return view('superadmin::subscription.max_sup_warehouse_count_modal');
            }
        }
        }  elseif ($type == 'users') {
            $response_array = ['success' => 0,
    ...
    public function availableModules()
    {
      $modules = [
        ...
        if (is_product_enabled('249')){
          $modules['tables']  = [ 'name' => __('restaurant.tables'),'tooltip' => __('restaurant.tooltip_tables')];
        }
        if (is_product_enabled('276')){
          $modules['subscription']  = ['name' => __('lang_v1.enable_subscription')];
        }
    ...
    public function assignPermissions($permissions){
      $role = Role::where('business_id',session('business.id'))->where('is_default',1)->first();
      $list =[];
      foreach($permissions as $key=>$value){
        $list[]= $value['value'];
      }
      $persmissions = Permission::whereIn('name', $list )->pluck('id')->toArray();
      // dd($role);
      //   dd($value['value']);
      $role->syncPermissions($persmissions);
      // dd($list);
