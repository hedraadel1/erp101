...
class TransactionUtil extends Util
{
  ...
  public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
  {
    ...
    $transaction = Transaction::create([
      ...
      'commission_user_login' => isset($input['commission_user_login']) ? $input['commission_user_login'] : null,
    ...
    foreach ($products as $product) {
      ...
      if (!empty($product['transaction_sell_lines_id'])) {
        ...
        $line = [
          ...
          'sell_line_serial_code' => !empty($product['sell_line_serial_code']) ? $product['sell_line_serial_code'] : '',
        ...
        //check request has serialcode
        if (!empty($product['sell_line_serial_code'])) {
          $serial_code = ProductSerial::where('business_id', session('business.id'))->where('serial_code', $product['sell_line_serial_code'])->first();
          $serial_code->is_used = '1';
          $serial_code->save();
        }
        // dd($line);
      ...
      if (!empty($payment['payment_id'])) {
        ...
        if ($payment_amount != 0) {
          ...
          $payment_data = [
            ...
            'chart_of_account' => !empty($payment['chart_of_account']) && $payment['method'] != 'advance' ? $payment['chart_of_account'] : null
          ];
  ...
  public function getTotalSellCommission($business_id, $start_date = null, $end_date = null, $location_id = null, $commission_agent = null)
  {
    ...
    $query = TransactionSellLine::leftjoin('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
      ...
      ->select('t.*');
    // ->select('t.*',DB::raw("SUM( (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price ) as all_final_total"));
    //Check for permitted locations of a user
    $permitted_locations = auth()->user()->permitted_locations();
    if ($permitted_locations != 'all') {
    ...
    if (!empty($commission_agent)) {
      ...
      $query->orWhere('t.commission_user_login', $commission_agent);
    }
    ...
    foreach ($sell_details as $sell) {
      $commission_user = User::where('id', $commission_agent)->first();
      // dd($sell->commission_agent);
      if ($sell->commission_agent == null || $sell->commission_user_login == null) {
        $commission_s =  $commission_user->cmmsn_percent2;
        $total += ($sell->final_total * $commission_s) / 100;
      } else {
        $commission_s =  $commission_user->cmmsn_percent;
        $total += ($sell->final_total * $commission_s) / 100;
      }
      $all_final_total += $sell->final_total;
    }
    // dd($total);
    // $output['total_sales_with_commission'] = $sell_details->sum('final_total');
    $output['total_sales_with_commission'] = $total;
    $output['all_final_total'] = $all_final_total;
  ...
  public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null)
  {
    ...
    foreach ($payments as $payment) {
      ...
      $ledger[] = [
        ...
        // 'purchase',
        // 
        'debit' => in_array($payment->transaction_type, ['sell_return', 'purchase'])   || $payment->payment_type == 'debit' || ($payment->is_advance == 1 && $contact->type == 'supplier' && $payment->card_type == 'debit') || $payment->card_type == 'debit' ? $payment->amount : '',
        'credit' => (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance'])) && $payment->is_return == 0 || (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance']) &&  $payment->is_return == 1) || $payment->payment_type == 'credit' || ($payment->card_type == 'credit' && $payment->transaction_type == null) ? $payment->amount : '',
        'others' =>  $note
      ];
    }
    ...
    if ($format != 'format_2') {
      ...
      $ledger = array_merge([[
        ...
        'total' => '',
        'payment_method' => '',
        'debit' => $contact->opening_balance_type == 'debit' ? abs($total_opening_bal) : '',
        'credit' =>  $contact->opening_balance_type == 'credit' ? abs($total_opening_bal) : '',
        'others' => '',
  ...
  public function updateExpense($request, $id, $business_id, $format_data = true)
  {
    ...
    $transaction_data['type'] = !empty($request->input('is_refund')) && $request->input('is_refund') == 1 ? 'c' : $transaction->type;;
  ...
  public function payContact($request, $format_data = true)
  {
    ...
    $inputs = $request->only([
      'location_id', 'amount', 'opening_balance_type', 'method', 'note', 'card_number', 'card_holder_name',
    ...
    $opening_balance_type = isset($inputs['opening_balance_type']) ? $inputs['opening_balance_type'] : '';
    if (isset($inputs['opening_balance_type'])) {
      unset($inputs['opening_balance_type']);
    }
    if ($opening_balance_type == 'debit') {
      $inputs['card_type'] = $opening_balance_type;
    }

    // dd($inputs);
    $parent_payment = TransactionPayment::create($inputs);
    if ($contact->type == 'customer' && $opening_balance_type == 'credit') {
      $this->map_to_chart_of_account('customer_payment', $parent_payment->id, true, 'is_customer_payments');
    } elseif ($contact->type == 'customer' && $opening_balance_type == 'debit') {
      $this->map_to_chart_of_account('customer_payment_debit', $parent_payment->id, true, 'is_customer_payments_debit');
    } elseif ($contact->type == 'supplier' && $opening_balance_type == 'credit') {
      $this->map_to_chart_of_account('supplier_payment', $parent_payment->id, true, 'is_supplier_payments');
    } elseif ($contact->type == 'supplier' && $opening_balance_type == 'debit') {
      $this->map_to_chart_of_account('supplier_payment_debit', $parent_payment->id, true, 'is_supplier_payments_debit');
    }
  ...
  public function map_to_chart_of_account($mapping_for, $transaction_id, $is_payment = false, $map_setting_type)
  {
    // DB::beginTransaction();
    $chart_of_accounts = AccountMapSetting::where('business_id', session('business.id'))->where($map_setting_type, '1')->get();
    // $account_id = TransactionPayment::where('id', $transaction_id)->select('account_id')->firstOrFail()  ;

    if (count($chart_of_accounts) > 0) {
      // dd($chart_of_accounts);
      foreach ($chart_of_accounts as $chart_of_account_id) {
        if ($chart_of_account_id) {
          $business = Business::where('id', session('business.id'))->select('currency_id')->firstOrFail();
          if ($is_payment == true) {
            $transaction = TransactionPayment::where('id', $transaction_id)->select('transaction_id', 'location_id', 'amount')->firstOrFail();
          } else {
          ...
          if ($is_payment == true) {
            if (isset($transaction->location_id)) {
              $journal_entry->location_id =  $transaction->location_id;
            }
            if (isset($transaction->transaction_id)) {
              $transaction_Location = Transaction::where('id', $transaction->transaction_id)->select('location_id')->firstOrFail();
              $journal_entry->location_id =  $transaction_Location->location_id;
            }
          } else {
            $journal_entry->location_id = $transaction->location_id;
          }
          ...
          $journal_entry->chart_of_account_id = $chart_of_account_id->chart_account_id;
          ...
          if ($is_payment == true) {
            $chart_of_account_id->type == 'credit' ?
  ...
  /**
   * this method make auto link all transaction with account 
   * @param transaction_payment_id   id of transaction payment
   * @param location_id 
   * @param map_setting_type type of account_settings (return sell or return purchase)
   */
  public function auto_link_account($transaction_payment_id, $location_id, $map_setting_type)
  {
    $business_id = session()->get('user.business_id');
    $payment_id = $transaction_payment_id;

    $accounts = AccountSetting::where('business_id', session('business.id'))->where('location_id', $location_id)->where($map_setting_type, '1')->get();
    if (!$accounts) {
      $accounts = AccountSetting::where('business_id', session('business.id'))->where($map_setting_type, '1')->get();
    }
  ...
  /**
   * this method make pay for contact auto in this case (return sell or return purchase)
   * @param contact_id   id of contacy
   * @param amount total amount will br paid
   * @param due_payment_type type of due payment transaction (return sell or return purchase)
   */
  public function autoPayContact($contact_id, $amount, $due_payment_type, $format_data = true)
  {
    $business_id = auth()->user()->business_id;
    // $inputs = $request->only(['amount','opening_balance_type', 'method', 'note', 'card_number', 'card_holder_name',
    //     'card_transaction_number', 'card_type', 'card_month', 'card_year', 'card_security',
    //     'cheque_number', 'bank_account_number']);
    $inputs = [];
    $payment_types = $this->payment_types();
    ...
    if ($format_data) {
      ...
      $inputs['amount'] = $this->num_uf($amount);
    }
   ...
   */
  public function sendNotificationWhatsapp($to, $message, $url = null)
  {
      $appKey = session('business.setting_appkey');
      $authkey = session('business.setting_authkey');
      
      // Initialize $response with a default value (null)
      $response = null;
      
      if (!empty($authkey) && !empty($appKey)) {
          $curl = curl_init();
          curl_setopt_array($curl, array(
              CURLOPT_URL => 'https://aw.onoo.pro/public/api/create-message',
          ...
          // Store the response from the cURL request in $response
          $response = curl_exec($curl);
  
          curl_close($curl);
      }
  
      // Return the value of $response (even if it's null)
      return $response;
  }
   ...
   * auto send whatsapp messages 
   */
  public function sendNotificationWhatsapptemplate($to, $url = null)
  {
    $appKey = session('business.setting_appkey');
    $authkey = session('business.setting_authkey');
    // dd($message);
    if (!empty($authkey) && !empty($appKey)) {
      $curl = curl_init();
      curl_setopt_array($curl, array(
        CURLOPT_URL => 'http://aw.onoo.pro/public/api/create-message',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
        ...
        CURLOPT_POSTFIELDS => array(
          ...
          'to' => $to,
          'message' => '----------------------------------
          تهنئ شركة اونو للبرمجيات 
          ----------------------------------
          عملائنا الكلام بعيد الاضحي المبارك 
          اعادة الله علينا وعليكم بالخير واليمن والبركاته 
