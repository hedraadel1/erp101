...
if (! function_exists('str_ordinal')) {
  ...
  if (! function_exists('package')) {
    ...
    function package()
    {
      $package = \Modules\Superadmin\Entities\Subscription::active_subscription(session('business.id'));
      return $package;
    }
  }
  if (! function_exists('content')) {
    ...
    function content()
    {
      $content = \Modules\Superadmin\Entities\SuperadminContent::first()??new SuperadminContent();
      return $content;
    }
  }


  if (! function_exists('products_requried')) {
    function products_requried()
    {
      $product_ids = DB::table('hide_product_users')->where('user_id', auth()->user()->id)->pluck('product_id')->toArray();

      $products_unique = \Modules\Superadmin\Entities\SuperadminProduct::whereNotIn('id' , $product_ids)->where('is_unique',1)->get();
      return $products_unique;
    }
  }

  if (! function_exists('user_content')) {

    function user_content()
    {
      $user_content = DB::table('content_users')->where('user_id' , auth()->user()->id)->first();
      return $user_content ? true : false;
    }
  }
  ...
  if (! function_exists('is_admin')) {
    ...
    function is_admin()
    {
      $admin = \App\User::where('business_id',session('business.id'))->first();
      return $admin->id == auth()->user()->id ? true :false;
    }
  }
  if (! function_exists('getSetting')) {

     function getSetting($screen)
    {
        return  UserSettingScreen::where('user_id',auth()->user()->id)->where('screen', $screen)->first()->data_json ?? null;
...
if (! function_exists('is_product_enabled')) {
  function is_product_enabled($code)
  {
    $module_util = new ModuleUtil();
    //chech code enable or no
    $is_enabled = (bool)$module_util->hasThePermissionInSubscription(session('business.id'), $code);
    //get item with type ultimate pay
    $ultimate_product = SuperadminBusinessProduct::where('business_id',session('business.id'))->where('product_code', $code)->where('paid_type','ultimate')->first();
      if (!$ultimate_product) {
        $product = SuperadminBusinessProduct::where('business_id',session('business.id'))->where('product_code', $code)->where('paid_type','day')->first();
        if ($product) {
          $date = date('Y-m-d');
          if($product->end_date < $date) {
              updatePakage($product);
              $is_enabled = false;
          }
        }
      }    
...
if (! function_exists('enable_view_brand_store')) {
  function enable_view_brand_store()
  {
