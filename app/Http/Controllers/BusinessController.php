...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name','category_id', 'start_date', 'currency_id', 'time_zone']);
  ...
  public function getBusinessSettingsWizard()
  {
      ...
      $weighing_scale_setting = !empty($business->weighing_scale_setting) ? $business->weighing_scale_setting : null;
      $dashboard_settings = !empty($business->dashboard_settings) ?json_decode($business->dashboard_settings, true) : [];
      $password_settings = !empty($business->password_settings) ?json_decode($business->password_settings , true): [];

      $payment_types = $this->moduleUtil->payment_types(null, false, $business_id);
      
      $enable_installment = false;
      if ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'enable_installment')) {
      ...
      return view('business.SettingWizard', compact( 'whastapp_setting','products','enable_installment','business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types' ,'dashboard_settings','password_settings'));
  }
    ...
    public function getBusinessSettings()
    {
        ...
        $enable_installment = false;
        if ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'enable_installment') || is_product_enabled('224')) {
          $enable_installment = true;
        }
        $product_ids = SuperadminBusinessProduct::where('business_id', $business_id)->pluck('product_id')->toArray();
        $products = SuperadminProduct::whereIn('id',$product_ids)->latest()->get();
        ...
        return view('business.settings', compact( 'whastapp_setting','products','enable_installment','business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types' ,'dashboard_settings','password_settings'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'time_format', 'ref_no_prefixes', 'theme_color', 'email_settings', 'dashboard_settings','password_settings',
                'sms_settings', 'rp_name', 'amount_for_unit_rp', 
                'min_order_total_for_rp', 'max_rp_per_order','setting_appkey','setting_authkey',
            ...
            if (!empty($request->input('enable_view_short_menu')) &&  $request->input('enable_view_short_menu') == 1) {
                $business_details['enable_view_short_menu'] = 1;
            } else {
                $business_details['enable_view_short_menu'] = 0;
            }
            ...
            $checkboxes = ['enable_editing_product_from_purchase',
                'enable_inline_tax',
                'enable_brand', 'enable_category','enable_category2','enable_category3', 'enable_sub_category', 'enable_price_tax', 'enable_purchase_status',
            ...
            $business_details['custom_labels'] = isset($business_details['custom_labels'])?json_encode($business_details['custom_labels']):null;
            $business_details['dashboard_settings'] = isset($business_details['dashboard_settings'])?json_encode($business_details['dashboard_settings']): null;
            $business_details['password_settings'] = isset($business_details['password_settings'])?json_encode($business_details['password_settings']): null;
    ...
    /***
     * check password setting of some route
     */
    public function checkPasswordSetting(Request $request){
      $password_settings = json_decode(session('business.password_settings'),true);
      // dd($request->all());
      // if($request->page_type == 'setting_page'){
        $output= false;
        if (isset($password_settings[$request->check_pass] ) && $password_settings[$request->check_pass] != null) {
          if ($password_settings[$request->check_pass] ) {
            $output= true;
          } 
        }
     ...
     * check password setting of some route
     */
    public function confirmPasswordSetting(Request $request){
      $password_settings = json_decode(session('business.password_settings'),true);
      // dd($request->all());
      // if($request->page_type == 'setting_page'){
        $output= false;
        if (isset($password_settings[$request->page_type] ) && $password_settings[$request->page_type] != null) {
          if ($request->password == $password_settings[$request->page_type] ) {
            $output= true;
          } 
        }
        return response()->json($output);
