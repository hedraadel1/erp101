  ...
  public function __construct(
	...
	public function handle($request, Closure $next)
	{
		...
		Menu::create('admin-sidebar-menu', function ($menu) {
			...
			if (!empty($pos_settings['installment_sales']) && auth()->user()->can('view_istallment_dashboard')) {
        $menu->url(action('InstallmentReport@getAllClientsReport'),'لوحة الاقساط', ['icon' => 'homeimg', '' => request()->segment(1) == 'home'])->order(0);
      }
            ...
            if (auth()->user()->can('essentials.enable_supervisor') && is_product_enabled('239')) {
            $sub->url(
              action('SupervisorController@index'),
              __('lang_v1.supervisors_section'),
              ['icon' => 'fa fas fa-user', 'active' => request()->segment(1) == 'users']
            );
          }
						if (auth()->user()->can('user.create') && is_product_enabled('240')) {
							$sub->url(
						...
						if ((auth()->user()->can('customer.view') || auth()->user()->can('customer.view_own'))&& is_product_enabled('223')) {
							$sub->url(
								action('AgentController@index'),
								__('lang_v1.traking_agent'),
								['icon' => 'fa fas fa-star', 'active' =>  request()->segment(1) == 'agents']
							);
						}
      ...
      $sub->url(
        ...
        ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'warehouses']
      );
      $sub->url(
        action('WarehouseInventoryController@index'),
        __('lang_v1.warehouses_inventory'),
        ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'warehouse-inventory']
      );
			...
			if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
				$menu->dropdown(
					...
					function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
						...
						if (($is_admin || auth()->user()->hasAnyPermission(['access_shipping', 'access_own_shipping', 'access_commission_agent_shipping']))&& is_product_enabled('244')) {
							$sub->url(
			...
			if (
				...
				$menu->dropdown(
					...
					function ($sub) use ($enabled_modules, $is_admin) {
						...
						if (config('constants.show_report_606') == true || is_product_enabled('259')) {
							$sub->url(
						...
						if ((in_array('purchases', $enabled_modules) || in_array('add_sale', $enabled_modules) || in_array('pos_sale', $enabled_modules) || is_product_enabled('258')) && auth()->user()->can('purchase_n_sell_report.view')) {
							$sub->url(
						...
						if (auth()->user()->can('trending_product_report.view') && is_product_enabled('260')) {
							$sub->url(
      ...
      if (!auth()->user()->can('superadmin')){
				$menu->url(action('SupportController@index'), __('lang_v1.support'), ['icon' => 'fa fa-globe', 'active' => request()->segment(1) == 'supported'])->order(99);
			}
      //support_admin_tickets menu
      if (auth()->user()->can('superadmin')){
				$menu->url(action('SupportAdminController@index'), __('lang_v1.support'), ['icon' => 'fa fa-globe', 'active' => request()->segment(1) == 'supportadmin'])->order(99);
			}
			...
			//Booking menu
			if ((in_array('booking', $enabled_modules) || is_product_enabled('226')) && (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) {
			...
			//Service Staff menu
			if (in_array('service_staff', $enabled_modules) && auth()->user()->can('view_restaurant_order')) {
            ...
            //Notification template menu
						...
						if (auth()->user()->can('send_notifications') && $this->moduleUtil->hasThePermissionInSubscription(session('business.id'), 'enable_notice_forms')) {
							$sub->url(
