...
class User extends Authenticatable
{
    ...
    {
        return $this->hasMany(\App\SupervisorUser::class , 'supervisor_id' , 'id');
    }
     ...
     * @param $check_is_supervisor = false (boolean)
     *
     * @return array users
     */
    public static function forDropdown($business_id, $prepend_none = true , $include_commission_agents = false, $prepend_all = false, $check_location_permission = false , $check_supervisor = false)
    {
    ...
    */
    ...
    {
      $user = Agent::where('user_id' , $this->id)->first();
    ...
    public function get_salary(){
      ...
      $end_date = Carbon::parse($start_date)->lastOfMonth()->format('Y-m-d');
      // dd($end_date);
      $total_work_duration = 0;
            $query = EssentialsAttendance::where('business_id',session('business.id'))
            ...
            $minutes_sum = $query->select(FacadesDB::raw('SUM(TIMESTAMPDIFF(MINUTE, clock_in_time, clock_out_time)) as total_minutes'))->first();
            $total_work_duration = !empty($minutes_sum->total_minutes) ? $minutes_sum->total_minutes / 60 : 0;
        return number_format($total_work_duration, 2) * $this->essentials_salary;
    }
