...
class Contact extends Authenticatable
{
    ...
    */
    ...
    {
        return $this->hasMany(\App\Contact::class , 'contact_id' ,'id');
    }
    ...
    */
    public function scopeOnlyCustomers($query)
    {
        ...
        if (auth()->check() && !auth()->user()->can('customer.view') && auth()->user()->can('customer.view_own')) {
            ...
            $query->where( function($q){
                $user_id = auth()->user()->id;
                $user_group_ids = UserGroup::where('user_id',auth()->user()->id)->pluck('group_id');
                $q->where('contacts.created_by', $user_id)
                    ->orWhere('ucas.user_id', $user_id)
                    ->whereIn('contacts.group_id', $user_group_ids);
   ...
   public function getLastBalance($id){
    $setting = session('business.common_settings');
    $data = null;
    if (!empty($setting['show_final_balance'])) {
     
    $date = Carbon::now()->format('Y');
    $start_date= $date.'-01-01';
    $end_date= $date.'-12-31';
    $transactionUtil = new TransactionUtil();
        $ledger_details = $transactionUtil->getLedgerDetails($id, $start_date, $end_date ,'format_1' ,null);
    $data = end($ledger_details['ledger'])['balance'] ?? null;
