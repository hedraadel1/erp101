                ...
                @component('components.filters', ['title' => __('report.filters')])
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            {!! Form::select('service_staff_id', $waiters, null, [
                                'class' => 'form-control select2',
                                'style' => 'width:100%',
        ...
        $(document).ready(function() {
            ...
            service_staff_report = $('table#service_staff_report').DataTable({
                ...
                "ajax": {
                    "url": "/sells",
                    "data": function(d) {
                        var start = $('input#ssr_date_range').data('daterangepicker').startDate.format(
                            'YYYY-MM-DD');
                        var end = $('input#ssr_date_range').data('daterangepicker').endDate.format(
                            'YYYY-MM-DD');
                ...
                "fnDrawCallback": function(oSettings) {
                    $('#footer_total_amount').text(sum_table_col($('#service_staff_report'),
                        'final-total'));
                    $('#footer_subtotal').text(sum_table_col($('#service_staff_report'),
                        'total_before_tax'));
                    $('#footer_total_tax').text(sum_table_col($('#service_staff_report'), 'total-tax'));

                    $('#footer_total_discount').text(sum_table_col($('#service_staff_report'),
                        'total-discount'));
            ...
            service_staff_line_orders = $('table#service_staff_line_orders').DataTable({
                ...
                "fnDrawCallback": function(oSettings) {
                    ...
                    $('#sslo_total_tax').text(sum_table_col($('#service_staff_line_orders'),
                        'item_tax'));
                    $('#sslo_unit_price').text(sum_table_col($('#service_staff_line_orders'),
                        'unit_price_before_discount'));
                    $('#sslo_total_discount').text(sum_table_col($('#service_staff_line_orders'),
                        'total-discount'));

                    $('#sslo_subtotal').text(sum_table_col($('#service_staff_line_orders'),
                        'unit_price_inc_tax'));
