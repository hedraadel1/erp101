...
@section('content')
    ...
    <head>
        <link rel="stylesheet" href="{{ asset('css/productbutton.css') }}">
    ...
    <section class="content">
        ...
        @if (isMobile())
        @else
            <div style="margin-left:unset !important;margin-right:unset !important;" class="row">
                <div class="btngroupproduct-group">
                    <div class="col-sm-3">
                        @if ($selling_price_group_count)
                            <button type="submit" value="submit_n_add_selling_prices"
                                class="btngroupproduct btngroupproduct-lg btn-red submit_product_form">@lang('lang_v1.save_n_add_selling_price_group_prices')</button>
                        @endif
                    </div>
                    <div class="col-sm-3">
                        @can('product.opening_stock')
                            <button type="submit" @if (empty($product->enable_stock)) disabled="true" @endif
                                ...
                                class="btngroupproduct submit_product_form btngroupproduct-lg btn-green">@lang('lang_v1.update_n_edit_opening_stock')</button>
                        @endcan
                    </div>
                    <div class="col-sm-3">
                        <button type="submit" value="save_n_add_another"
                            class="btngroupproduct submit_product_form btngroupproduct-lg btn-blue">@lang('lang_v1.update_n_add_another')</button>
                    </div>
                    <div class="col-sm-3">
                        <button type="submit" value="submit"
                            class="btngroupproduct btngroupproduct-lg submit_product_form btngroupproduct-light">@lang('messages.update')</button>
                    </div>
                </div>
            </div>
        @endif
        <input type="hidden" id="product_id" value="{{ $product->id }}">
        @component('components.widget', ['class' => 'box-primary', 'title' => __('product.edit_product')])
            <div class="row">
                <div class="col-sm-4">
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        {!! Form::text('sku', $product->sku, ['class' => 'form-control', 'placeholder' => __('product.sku')]) !!}
                    </div>
                </div>
                <div class="col-sm-4">
        ...
        <div class="w3-container">
            ...
            <div style="border-radius: 10px 10px 0 0;padding-right: 10px;padding-top: 2px;padding-left: 13px;padding-bottom: 2px;"
                class="w3-bar w3-black ">
                <div class="w3-bar-item width100 w3-button button-header-40 productbut tablink w3-red"
                    onclick="openCity(event,'Basic_info')">@lang('product.Basic_info')</div>
                <div class="w3-bar-item width100  w3-button button-header-40 productbut tablink"
                    onclick="openCity(event,'addon_info')">
                    @lang('product.addon_info')</div>
                <div class="w3-bar-item width100 w3-button button-header-40 productbut tablink"
                    onclick="openCity(event,'price_info')">
                    @lang('product.price_info')</div>
            </div>

            <div id="Basic_info" class="w3-container w3-border city">

                {{--     @component('components.widget', ['class' => 'box-primary', 'title' => __('lang_v1.all_sales')]) --}}

                <div class="row boxcontain">
                                ...
                                <select name="sub_unit_ids[]" class="form-control select2" multiple id="sub_unit_ids">
                                    @foreach ($sub_units as $sub_unit_id => $sub_unit_value)
                                        <option value="{{ $sub_unit_id }}"
                                            @if (is_array($product->sub_unit_ids) && in_array($sub_unit_id, $product->sub_unit_ids)) selected @endif>
                    ...
                    {{-- Added two new dropDowns for cat2 and cat3 --}}
                    <div class="col-sm-4 @if (session('business.enable_category2') == 0) hide @endif">
                        <div class="form-group">
                            {!! Form::label('category2_id', __('Category 2') . ':') !!}
                            {!! Form::select('category2_id', $categorys2, $product->category2_id, [
                                'placeholder' => __('messages.please_select'),
                                'class' => 'form-control select2',
                            ]) !!}
                        </div>
                    ...
                    <div class="col-sm-4 @if (session('business.enable_category3') == 0) hide @endif">
                        <div class="form-group">
                            {!! Form::label('category3_id', __('Category 3') . ':') !!}
                            {!! Form::select('category3_id', $categorys3, $product->category3_id, [
                                'placeholder' => __('messages.please_select'),
                                'class' => 'form-control select2',
                            ]) !!}
                        </div>
        ...
        <div id="addon_info" class="w3-container w3-border city" style="display:none">
        ...
        <div id="price_info" class="w3-container w3-border city" style="display:none">
            ...
            <div class="row boxcontain">
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        {!! Form::select('type', $product_types, $product->type, [
                            'class' => 'selectwidth form-control select2',
                            'required',
                            'disabled',
                            'data-action' => 'edit',
                ...
                <div class="col-sm-4 @if (!session('business.enable_price_tax')) hide @endif">
                    <div class="form-group">
                        {!! Form::label('tax', __('product.applicable_tax') . ':') !!} <br>
                        {!! Form::select(
                            'tax',
                            $taxes,
                            $product->tax,
                            ['placeholder' => __('messages.please_select'), 'class' => 'selectwidth form-control select2'],
                            $tax_attributes,
                        ) !!}
                    </div>
                </div>
                ...
                <div class="col-sm-4 @if (!session('business.enable_price_tax')) hide @endif">
                    <div class="form-group">
                        {!! Form::label('tax_type', __('product.selling_price_tax_type') . ':*') !!} <br>
                        {!! Form::select(
                            'tax_type',
                            ['inclusive' => __('product.inclusive'), 'exclusive' => __('product.exclusive')],
                            $product->tax_type,
                            ['class' => 'form-control select2 selectwidth', 'required'],
                        ) !!}
                    </div>
                </div>
                ...
                @if ($product->producDiscount()->count() > 0)
                    @include('product.partials.edit_discount_product_form_part', [
                        'discount_types' => $discount_types,
                        'product_discount' => $product->producDiscount()->get(),
                    ])
                @else
                    <button style="width:100%" onclick="$('table.add_discount_product').toggleClass('hide')"
                        type="button" class="btn btn-info">{{ __('product.product_price_discounts_button') }}</button>
                    <br>
                    @include('product.partials.discount_product_form_part', [
                        'discount_types' => $discount_types,
                    ])
                @endif
            </div>
        ...
        {{-- @if (isMobile())
            ...
            <div style="margin-left:unset !important;margin-right:unset !important;" class="row">
                <div class="col-sm-3">
                    @if ($selling_price_group_count)
                        <button type="submit" value="submit_n_add_selling_prices"
                            class="btn width100 btn-warning submit_product_form">@lang('lang_v1.save_n_add_selling_price_group_prices')</button>
                    @endif
                </div>
                ...
                <div class="col-sm-3">
                    @can('product.opening_stock')
                        <button id="opening_stock_button" @if (!empty($duplicate_product) && $duplicate_product->enable_stock == 0) disabled @endif type="submit"
                            ...
                            class="btn width100 bg-purple submit_product_form">@lang('lang_v1.save_n_add_opening_stock')</button>
                    @endcan
                </div>
                <div class="col-sm-3">

                    <button type="submit" value="save_n_add_another"
                        class="btn width100 bg-maroon submit_product_form">@lang('lang_v1.save_n_add_another')</button>

                </div>
                <div class="col-sm-3">

                    <button type="submit" value="submit"
                        class="btn width100 btn-primary submit_product_form">@lang('messages.save')</button>
            ...
            <div style="margin-left:unset !important;margin-right:unset !important;" class="row">
                ...
                <div class="btngroupproduct-group">
                    ...
                    <div class="col-sm-3">
                        @if ($selling_price_group_count)
                            <button type="submit" value="submit_n_add_selling_prices"
                                class="btn btngroupproduct btngroupproduct-lg btn-red submit_product_form ">@lang('lang_v1.save_n_add_selling_price_group_prices')</button>
                        @endif
                    </div>

                    <div class="col-sm-3">
                        @can('product.opening_stock')
                            <button value="submit_n_add_opening_stock" id="opening_stock_button"
                                @if (!empty($duplicate_product) && $duplicate_product->enable_stock == 0) disabled @endif type="submit"
                                class="btn btngroupproduct submit_product_form btngroupproduct-lg btn-green">@lang('lang_v1.save_n_add_opening_stock')</button>
                        @endcan
                    </div>

                    <div class="col-sm-3">
                        <button type="submit" value="save_n_add_another"
                            class="btn btngroupproduct submit_product_form btngroupproduct-lg btn-blue">@lang('lang_v1.save_n_add_another')</button>
                    ...
                    <div class="col-sm-3">
                        <button type="submit" value="submit"
                            class="btn btngroupproduct btngroupproduct-lg submit_product_form btngroupproduct-light">@lang('messages.save')</button>
                    ...
                    <div class="col-sm-3">
                        @can('product.opening_stock')
                            <button id="opening_stock_button" type="submit" value="submit_n_add_opening_stock"
                                class="btn width100 bg-purple submit_product_form">@lang('lang_v1.update_n_edit_opening_stock')</button>
                        @endcan
                    </div>
                    <div class="col-sm-3">

                        <button type="submit" value="save_n_add_another"
                            class="btn width100 bg-maroon submit_product_form">@lang('lang_v1.update_n_add_another')</button>

                    </div>
                    <div class="col-sm-3">

                        <button type="submit" value="submit"
                            class="btn width100 btn-primary submit_product_form">@lang('messages.update')</button>
