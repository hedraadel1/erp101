...
@section('content')
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-12">
                @component('components.filters', ['title' => __('report.filters')])
                    ...
                    @endphp
                    {{--      Added two new dropmenus for category2 and category3 filteration --}}
                    @if ($settings->enable_category2 == 1)
                        <div class="col-md-3">
                            <div class="form-group">
                                {!! Form::label('category2_id', __('product.category') . ' 2:') !!}
                                {!! Form::select('category_id', $categorys2, null, [
                                    'class' => 'form-control select2',
                                    'style' => 'width:100%',
                                    'id' => 'product_list_filter_category2_id',
                                    'placeholder' => __('lang_v1.all'),
                                ]) !!}
                            </div>
                        </div>
                    ...
                    @if ($settings->enable_category3 == 1)
                        <div class="col-md-3">
                            <div class="form-group">
                                {!! Form::label('category3_id', __('product.category') . ' 3:') !!}
                                {!! Form::select('category_id', $categorys3, null, [
                                    'class' => 'form-control select2',
                                    'style' => 'width:100%',
                                    'id' => 'product_list_filter_category3_id',
                                    'placeholder' => __('lang_v1.all'),
                                ]) !!}
                            </div>
                        </div>
                    ...
                    <div class="col-md-3">
                        {!! Form::label('Product_Status', __('lang_v1.isactive') . ':') !!}
            ...
            @if (is_product_enabled('271'))
                <div class="col-md-12">
                    @include('product.partials.short-statistics')
                </div>
            @endif

        </div>

        @component('components.widget', ['class' => 'box-primary', 'title' => __('lang_v1.products')])
            <div class="row">
                <div class="col-md-6">
                    <a class="button-add" href="{{ action('ProductController@create') }}">
                ...
                <div class="col-md-6">
                    @can('product.create')
                        @if ($is_admin)
                            <a class="button-add" href="{{ action('ProductController@downloadExcel') }}"><i
                                    class="fa fa-download maricon"></i>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="nav-tabs-custom">
                            <ul class="nav nav-tabs">
                                <li style="width: {{ isMobile() ? '100%' : '48%' }};text-align: center;" class="active">
                                ...
                                @can('stock_report.view')
                                    <li style="width:  {{ isMobile() ? '100%' : '50%' }};text-align: center;">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="product_list_tab">
                                    ...
                                    {{-- <button type="button" class="btn btn-xs btn-primary setting_screen_table">
                                        اعدادات عرض الاعمدة
                                    </button> --}}
        ...
        @include('product.partials.increase_product_price_modal', [
            'categories' => $categories,
            'brands' => $brands,
        ])
        @include('product.partials.screen_setting_modal')
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            product_table = $('#product_table').DataTable({
                ...
                "ajax": {
                    ...
                    "data": function(d) {
                        ...
                        d.category2_id = $('#product_list_filter_category2_id').val();
                        d.category3_id = $('#product_list_filter_category3_id').val();
            ...
            $(document).on('change',
                '#product_list_filter_type, #product_list_filter_category_id,#product_list_filter_category2_id,#product_list_filter_category3_id ,#product_list_filter_brand_id, #product_list_filter_unit_id, #product_list_filter_tax_id, #location_id, #active_state, #repair_model_id',
                function() {
    ...
    <script>
        ...
        $(document).ready(function() {
            $(window).on('load', function() {
                $.ajax({
                    url: "{{ route('getscreen_setting', 'products') }}",
                    type: 'get',
                    dataType: 'json',
                    success: function(res) {
                        ...
                        $('.dt-button-collection  li').each(function(index, element) {
                            if (cols) {
                                if (cols[index]) {
                                    cols[index].display == '1' ? $(element).click() :
                                        null;
                                } else {
                                    // $(element).click()
                                }
        ...
        $(document).on('change',
            '#product_list_filter_type, #product_list_filter_category_id, #product_list_filter_brand_id, #product_list_filter_unit_id',
            '#product_list_filter_tax_id, #active_state, #location_id',
            function() {
                var type = null;
                var tax_id = null;
                ...
                if ($('#product_list_filter_type').length > 0) {
                    type = $('#product_list_filter_type').val();
                }
                if ($('#product_list_filter_brand_id').length > 0) {
                    brand_id = $('#product_list_filter_brand_id').val();
                }
                if ($('#product_list_filter_unit_id').length > 0) {
                    unit_id = $('#product_list_filter_unit_id').val();
                }
                if ($('#product_list_filter_category_id').length > 0) {
                ...
                getStatistics(type, tax_id, active_state,
                    location_id, brand_id, unit_id, category_id);
            });

        function getStatistics(type = null, tax_id = null, active_state = null,
            location_id = null, brand_id = null, unit_id = null, category_id = null
        ) {
            $.ajax({
                url: "{{ route('product.getStatistics') }}",
                type: 'get',

                dataType: 'json',
                beforeSend: function() {
                    $('#product_count').html('');
                    $('#product_total_sell').html('');
                    $('#product_total_purchase').html('');
                    $('#product_count_finished').html('');
                    $('#category_count').html('');
                    $('#product_count_expire').html('');

                    $('#product_count').append(
                        '<i class="fas fa-sync fa-spin fa-fw margin-bottom"></i>');
                    $('#product_total_sell').append(
                        '<i class="fas fa-sync fa-spin fa-fw margin-bottom"></i>');
                    $('#product_total_purchase').append(
                        '<i class="fas fa-sync fa-spin fa-fw margin-bottom"></i>');
                    $('#category_count').append('<i class="fas fa-sync fa-spin fa-fw margin-bottom"></i>');
                    $('#product_count_expire').append(
                        '<i class="fas fa-sync fa-spin fa-fw margin-bottom"></i>');
                    $('#product_count_finished').append(
                        '<i class="fas fa-sync fa-spin fa-fw margin-bottom"></i>');
                },
                data: {
                    'type': type,
                ...
                success: function(res) {
                    $('#product_total_purchase').html(res.product_total_purchase);
                    $('#product_count').html(res.product_count);
                    $('#product_count_finished').html(res.product_count_finished);
                    $('#product_count_expire').html(res.product_count_expire);
                    $('#category_count').html(res.category_count);
                    $('#product_total_sell').html(res.product_total_sell);
