...
<body >
    ...
    <div class="modal-body">
      ...
      <div class="row">
          ...
          <div class="col-xs-6">
            @php
              $total = $contact_details->total_purchase == 0.0000 ? $contact_details->total_sell : $contact_details->total_purchase;
              $total_paid = $contact_details->purchase_total_paid == 0.0000 ? $contact_details->sell_total_paid : $contact_details->purchase_total_paid;
             // $old_due = $contact_details->opening_balance;
            @endphp
            ...
            @format_currency($total)<br>
           {{-- <strong>@lang('contact.total_paid') :</strong>
             @format_currency($total_paid)<br> --}}
            <strong>@lang('account.customer_due_old') :</strong>
            @format_currency($dues + $single_payment_line->amount)<br>
            {{-- @format_currency($old_due + $single_payment_line->amount)<br> --}}
            {{-- @format_currency(($total - $total_paid) + $single_payment_line->amount)<br> --}}
            {{-- @format_currency($transaction->contact->total_sell_return - $transaction->contact->total_return_paid)<br> --}}
            <strong>@lang('account.customer_due_new') :</strong>
            @format_currency($dues)<br>
            {{-- @format_currency($old_due)<br> --}}
            {{-- @format_currency(($total - $total_paid) == $total ? ($total - $single_payment_line->amount) : ($total - $total_paid))<br> --}}
            {{-- ------------------ --}}
            <hr>
