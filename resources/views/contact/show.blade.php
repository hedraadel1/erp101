...
@section('content')
    ...
    <section class="content no-print">
        ...
        <div style="margin-top: -25px;display:grid;height: unset;" class="newbox blackline">
            <span style="display: flex;align-items: center;justify-content: space-between;">
                <h3 style="margin-top: 10px;margin-bottom: 10px;"> {{ $contact->full_name_with_business }} </h3>
                <button title="معلومات الجهة" data-toggle="modal"data-target="#contact_details" class="btn btn-sm btn-info"
                    style="margin-{{ isRtl() ? 'right' : 'left' }}: 20px;">
                    <i class="fas fa-file-alt" style=""></i> <i class="fas fa-user-tie" style=""></i> </button>

                @if ($contact->type == 'customer' || $contact->type == 'both')
                    @if ($contact->total_invoice - $contact->invoice_received > 0)
                        <a style="margin: 0px 5px"
                            href="{{ action('TransactionPaymentController@getPayContactDue', [$contact->id]) }}?type=sell"
                            class="pay_purchase_due btn btn-primary btn-sm "><i class="fas fa-money-bill-alt"
                                aria-hidden="true"></i>
                        </a>
                    @endif
                @endif
                <button style="margin: 0px 5px" type="button" class="btn btn-primary btn-sm pull-right" data-toggle="modal"
            ...
            <div style="margin-top: -24px;">
                ...
                @if ($contact->type == 'both')
                    <div style=" background: blueviolet;width: 200px;text-align: center;border-radius: 7px;color: white;">
                        {{ __('role.customerlabel') }} & {{ __('role.supplierlabel') }}
                    </div>
                @elseif ($contact->type == 'supplier')
                    <div style=" background: coral;width: 200px;text-align: center;border-radius: 7px;color: white;">
                        {{ __('role.supplierlabel') }}
                    </div>
                @elseif($contact->type == 'customer')
                    <div style=" background: darkblue;width: 200px;text-align: center;border-radius: 7px;color: white;">
                        {{ __('role.customerlabel') }}
                    </div>
                @endif
            ...
            <div class="col-md-12">
                <div class="box box-solid">
                    <div class="box-body">
                        ...
                        @include('contact.partials.contact_section_statistics')
            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs nav-justified">
                        <li
                            ...
                            <a href="#ledger_tab" data-toggle="tab" aria-expanded="true"><i class="fas fa-scroll"
                                    aria-hidden="true"></i> @lang('lang_v1.ContactLedger')</a>
                        </li>
                        @if (in_array($contact->type, ['both', 'supplier']))
                            ...
                            @else
                                ...
                                <a href="#stock_report_tab" data-toggle="tab" aria-expanded="true"><i
                                        class="fas fa-hourglass-half" aria-hidden="true"></i> @lang('lang_v1.ContactStock')</a>
                            </li>
                        @endif
                        ...
                        @if (is_product_enabled('241'))
                            ...
                            ">
                                <a href="#documents_and_notes_tab" data-toggle="tab" aria-expanded="true"><i
                                        class="fas fa-paperclip" aria-hidden="true"></i> @lang('lang_v1.Contact_documents_and_notes')</a>
                            </li>
                        @endif
                        ...
                        @if (in_array($contact->type, ['both', 'customer']) && is_product_enabled('224'))
                            <li
                                class="
                          @if (!empty($view_type) && $view_type == 'ins') active
                          @else
                            '' @endif">
                                <a href="#ins_tab" data-toggle="tab" aria-expanded="true"><i
                                        class="fas fa-arrow-circle-up" aria-hidden="true"></i> @lang('lang_v1.ins')</a>
                            </li>
                        @endif
                        ...
                        @else
                            ...
                            <a href="#activities_tab" data-toggle="tab" aria-expanded="true"><i
                                    class="fas fa-pen-square" aria-hidden="true"></i> @lang('lang_v1.Contactactivities')</a>
                        </li>

                        @if (!empty($contact_view_tabs) && is_product_enabled('225'))
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane" id="ins_tab">
                            <div id="contant_ins_table" style="height: 500px;overflow-y: scroll;"></div>
                            {{-- @include('contact.contact_ins') --}}
                        </div>
    ...
    <div class="modal fade" id="contact_details" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content ">
                <div class="modal-header">
                    <h4 class="modal-title"> {{ $contact->full_name_with_business }}</h4>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="" style="border: 1px dashed;padding: 8px">
                        ...
                        <div class="col-md-6" style="    padding-top: 10px;">
                            <div class="" style="border: 1px dashed;padding: 8px">
                                ...
                                <span class="text-muted">
                                    {{ $contact->alternate_number ?? 'غير معروف' }}
                                </span>
                            </div>
                        </div>
...
@section('javascript')
    <script type="text/javascript">
        ...
        $(document).one('shown.bs.tab', 'a[href="#ins_tab"]', function() {
            get_ins();
        })
        ...
        function get_ins(url = null) {
            if (!url) {
                url = "{{ action('ContactController@getins', [$contact->id]) }}";
            }
            $.ajax({
                url: url,
                dataType: 'html',
                success: function(result) {
                    $('#contant_ins_table').fadeOut(400, function() {
                        $('#contant_ins_table')
