...
@section('content')
    ...
    <section style="margin-top: -25px;" class="content-header">
        <div class="newbox orange">
            <h3 style="margin-top: 10px;margin-bottom: 10px;">@lang('purchase.add_purchase') <i class="fa fa-keyboard-o hover-q text-muted"
                    aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom"
                    data-content="@include('purchase.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title=""
                    title=""></i></h3>
    ...
    <section class="content">
        ...
        @component('components.widget', ['class' => 'box-primary', 'title' => __('messages.basicdata')])
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <span class="input-group-btn">
                                <button style="padding: 9px 14px;" type="button"
                                    class="btn btn-default bg-white btn-flat add_new_supplier" data-name="">
                                    <i class="fa fa-plus-circle text-primary fa-lg"></i>
                                </button>
                                {{-- // button show contact --}}
                                @if (auth()->user()->can('supplier.view') ||
                                        auth()->user()->can('supplier.view_own'))
                                    <div style="padding: 9px 14px;" id="link-show" type="button"
                                        class="btn btn-default bg-white btn-flat" data-name="">
                                        <i class="fa fa-eye"></i>
                                    </div>
                                    {{--    <div id="link-show"></div> --}}
                                @endif
                            </span>
                        </div>
                    </div>
                ...
                <div class="col-sm-3 {{ !auth()->user()->can('purchase.update_status')? 'hide': '' }}">
                    <div class="form-group">
                        {!! Form::label('status', __('purchase.purchase_status') . ':*') !!} @show_tooltip(__('tooltip.order_status'))
                        {!! Form::select('status', $orderStatuses, 'ordered', [
                            'class' => 'form-control select2',
                            'placeholder' => __('messages.please_select'),
                            'required',
                        ]) !!}
            ...
            <div class="row">
                <div class="col-sm-4">
                    <div id="showdshipping" role="button"
                        style="background-image: linear-gradient(#464d55, #25292e);color: #fff;" class="button-30">
                        @lang('messages.showshipping')</div>

                </div>
                <div class="col-sm-4">
                    <div id="showdiscountandtax" role="button"
                        style="background-image: linear-gradient(#464d55, #25292e);color: #fff;" class="button-30">
                        @lang('messages.showdiscount')</div>

                </div>
                <div class="col-sm-4">
                    <div id="showdpaymentwebgit" role="button"
                        style="background-image: linear-gradient(#464d55, #25292e);color: #fff;" class="button-30">
                        @lang('messages.showpayment')</div>

                </div>
            </div>
        @endcomponent

        @component('components.widget', ['class' => 'box-primary', 'title' => __('messages.productdata')])
            <div class="row">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="table-responsive">
                        <table style="font-size: 13px;font-weight: 900;"
        ...
        @component('components.widget', [
            'class' => 'box-primary',
            'display' => 'none',
            'wedgitid' => 'discountandtax',
            'title' => __('messages.showdiscount'),
        ])
            <div class="row">
                <div class="col-sm-12">
                    <table class="table">
        ...
        @component('components.widget', [
            ...
            'display' => 'none',
            'wedgitid' => 'shipping',
            'title' => __('messages.showshipping'),
        ])
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
        ...
        <div class="row">
            <div class="col-sm-12">
                <button type="button" id="submit_purchase_form"
                    class="btn button-29 pull-right btn-flat">@lang('messages.save')</button>
            </div>
        </div>
        {!! Form::close() !!}
    </section>
    <!-- quick product modal -->
    <div class="modal fade product_serial_modal" tabindex="-1" role="dialog" aria-labelledby="modalTitle"></div>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $('#showdiscountandtax').on('click', function(event) {
            $('#discountandtax').css({
                'display': 'block',
            });
        });
        ...
        $('#showdpaymentwebgit').on('click', function(event) {
            $('#paymentwebgit').css({
                'display': 'block',
            });
        });

        $('#showdshipping').on('click', function(event) {
            $('#shipping').css({
                'display': 'block',
            });
        });
        ...
        $('select#supplier_id').on('change', function() {
            $.ajax({
                ...
                success: function(re) {
                    var html =
                        `<a target="_blank" class="" href="` + re.url +
    ...
    {{-- <input type="hidden" id="pur_row_count" value="{{ session('row_count')}}"> --}}
    {{-- task-3 --}}
    <div class="modal fade" data-rowcount='0' id="myModal" role="dialog">
        @include('purchase.partials.purchase_weight_modal')
    </div>
    <script>
        $(document).on('click', '.add_input', function() {
            var html = ` <div class="row " style="padding-top:6px">
                            <div class="col-md-10">
                              <input type="text" name="serial_code[]" placeholder="ادخل السيريل" class="form-control">
                            </div>
                            <div class="col-md-2">
                                <button type="button" class="btn btn-danger remove_input">-</button>
                            </div>
                         </div>
                        `;
            $('.div_input').append(html);
        });
        $(document).on('click', '.remove_input', function() {

            $(this).parent().parent().remove();
        });
        $(document).on('submit', 'form#save_product_serial_form', function(e) {
            e.preventDefault();
            var form = $(this);
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success == true) {
                        $('div.product_serial_modal').modal('hide');
