...
<tr class="product_row" data-row_index="{{ $row_count }}"
    ...
    <td>
        ...
        @if (($edit_price || $edit_discount) && empty($is_direct_sell))
            <div title="@lang('lang_v1.pos_edit_product_price_help')">
                <span class="text-link text-info cursor-pointer" data="modal"
        ...
        @php
            ...
            if (!empty($product_discount)) {
                ...
                foreach ($product_discount as $item) {
                    if ($item->quantity_from <= $product->quantity_ordered && $item->quantity_to >= $product->quantity_ordered) {
                        $dicount_product = $item;
                    }
                }
            }
    ...
    <td>
        ...
        <div class="input-group input-number">
            ...
            <input style="{{ isMobile() ? 'width: 100px' : '' }}" type="text" data-min="1"
                onblur="cheakDiscount(this)"
                class="form-control pos_quantity check_quantity input_number mousetrap input_quantity"
                value="{{ @format_quantity($product->quantity_ordered) }}"
                data-product_id="{{ $product->product_id }}"data-price="{{ @num_format($unit_price_inc_tax) }}"
        ...
        @endif
        <input type="hidden" class="productRow" value="{{ $row_count }}">
...
<script>
    /**
     * function to handle clicking on Model
     * ahmed ramadan
     * 4-9-2023
     * */
    $('.cursor-pointer').click(function($event) {
        $event.preventDefault();
        var tr = $(this).parents('tr');
        let rowNumber = $(tr.find('.productRow')).val();
        console.log(rowNumber);
        let model = $($(this).data("target")).modal();
        console.log(model);
        $('form#add_pos_sell_formt').prepend(model).css('display', 'block');

    });
    // $('input.check_quantity').on('change', function() {
    //     cheakDiscount($(this));
    // });

    // function cheakDiscount(el) {
    //     var quantity = $(el).val();
    ...
    //     $.ajax({
    //         url: '{{ route('product.chechHaveDiscont') }}',
    //         type: "GET",
    //         dataType: "json",
    //         data: {
    //             'quantity': quantity,
    ...
    //                     var unit_price = (price - item.discount);
    //                     alert(unit_price);
    //                     // pos_total_row();
    //                     __write_number(element, unit_price);

    //                     var total_quantity = 0;
    //                     var price_total = unit_price * quantity;
    //                     tr.find('input.pos_unit_price_inc_tax').val(price_total)
    //                     // tr.find('span.pos_line_total_text').text(__currency_trans_from_en(
    //                     //     price_total, true));
    //                     // tr.find('input.pos_line_total').val(__currency_trans_from_en(
    //                     //     price_total, false));
    //                     // $('table#pos_table tbody tr').each(function() {
    //                     //     total_quantity = total_quantity + parseInt(tr.find(
    //                     //         'input.check_quantity').val());
    //                     // });
    ...
    //                     $('span#total_payable').html(__currency_trans_from_en(price_total, false));
    //                     //         pos_total_row();
    //                     //         var old = $('span.price_total').text();
    ...
    $('.check_seral').on('keyup', function() {
        $.ajax({
            url: "{{ action('ProductController@checkProductSerial') }}",
            method: 'GET',
            dataType: 'json',
            data: {
                'serial_code': $(this).val(),
