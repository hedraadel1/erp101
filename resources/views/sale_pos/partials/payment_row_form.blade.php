<div class="row">
    ...
    <div class="{{ $col_class }}">
        <div class="form-group ">
            {!! Form::label("amount_$row_index", 'المبلغ المدفوع من الاجمالى' . ':*') !!}
    ...
    @if (!empty($pos_settings['installment_sales']))
        <div class="form-group">
            <div class="form-check form-switch">
                <label class="form-check-label" for="active_installment"> امكانيه التقسيط : </label>
                <input class="form-check-input" name="active_installment" type="checkbox" id="active_installment"
                    value="1">
            </div>
        </div>
    @endif
    ...
    @if (
        ...
        <div class="clearfix"></div>
    @endif
    @if (!empty($accounts) && empty($pos_settings['enable_paid_for_Accounting_module']))
        <div class="{{ $col_class }}">
    ...
    @endif
    @if (!empty($chart_of_accounts) && !empty($pos_settings['enable_paid_for_Accounting_module']))
        <div class="{{ $col_class }}">
            <div class="form-group @if ($readonly) hide @endif">
                {!! Form::label("account_$row_index", __('lang_v1.payment_account') . ':') !!}
                <div class="input-group">
                    ...
                    {!! Form::select(
                        "payment[$row_index][chart_of_account]",
                        $chart_of_accounts,
                        !empty($payment_line['account_id']) ? $payment_line['account_id'] : '',
                        [
                            'class' => 'form-control select2 account-dropdown',
