$(document).ready(function ()
{
  ...
  if ($('#sr_date_filter').length == 1)
  {
    ...
    //instllment representative report -> Sales
    sr_instllment_report = $('table#sr_instllment_report').DataTable({
      processing: true,
      serverSide: true,
      fixedHeader: false,
      aaSorting: [[0, 'desc']],
      ajax: {
        url: '/installment/data',
        data: function (d)
        {
          var start = $('input#sr_date_filter')
            .data('daterangepicker')
      ...
      columns: [
        { data: 'contact_id', name: 'contact_id', searchable: true },
        { data: 'installment_value', name: 'installment_value' },
        { data: 'installment_duo_date', name: 'installment_duo_date' },
        { data: 'mobile' },
        { data: 'landline' },
      ...
      "createdRow": function (row, data, dataIndex)
      {
        ...
        else if (data.installment_duo_date <= date && data.status == 0)
        {
    ...
    all_agent_table = $('table#all_agent_table').DataTable({
      processing: true,
      serverSide: true,
      fixedHeader: false,
    ...
    details_agent_table = $('table#details_agent_table').DataTable({
      processing: true,
      serverSide: true,
      // aaSorting: [
      //   [0, 'desc']
      // ],
      ajax: {
        url: '/agents/showdetails/' + $('#resource_id').val(),
        data: function (d)
        {
    ...
    sr_sales_commission_report = $('table#sr_sales_with_commission_table').DataTable({
      ...
      columns: [
        ...
        { data: 'commission_user_login', name: 'commission_user_login' },
      ],
      columnDefs: [
        {
          searchable: false,
      ...
      fnDrawCallback: function (oSettings)
      {
        ...
        $('#footer_total_commission').text(
          sum_table_col($('#sr_sales_with_commission_table'), 'commission')
        );
    ...
    //Sales representative report -> Sales with sr_sales_with_customer_groupcommission
    sr_sales_with_customer_groupcommission_report = $('table#sr_sales_with_customer_groupcommission_table').DataTable({
      processing: true,
      serverSide: true,
      aaSorting: [[0, 'desc']],
      ajax: {
      ...
      columns: [
        ...
        { data: 'total_paid', name: 'total_paid' },
        { data: 'total_remaining', name: 'total_remaining' },
        { data: 'customer_commission_id', name: 'customer_commission_id' },
        // { data: 'commission_user_login', name: 'commission_user_login' },
      ],
      columnDefs: [
        {
      ...
      // {
      //   $('#footer_sale_total').text(
      //     sum_table_col($('#sr_sales_with_customer_groupcommission_table'), 'final-total')
      //   );

      //   $('#footer_total_paid').text(
      //     sum_table_col($('#sr_sales_with_customer_groupcommission_table'), 'total-paid')
      //   );
      //   $('#footer_total_commission').text(
      //     sum_table_col($('#sr_sales_with_customer_groupcommission_table'), 'commission')
      //   );

      //   $('#footer_total_remaining').text(
      //     sum_table_col($('#sr_sales_with_customer_groupcommission_table'), 'payment_due')
      //   );
      //   $('#footer_total_sell_return_due').text(
      //     sum_table_col($('#sr_sales_with_customer_groupcommission_table'), 'sell_return_due')
      //   );

      //   $('#footer_payment_status_count ').html(
      //     __sum_status_html($('#sr_sales_with_customer_groupcommission_table'), 'payment-status-label')
      //   );
      //   __currency_convert_recursively($('#sr_sales_with_customer_groupcommission_table'));
      //   __currency_convert_recursively($('#sr_sales_with_commission'));
      // },
    });
  ...
  $(
    '#psr_customer_group_id, #psr_filter_category_id, #psr_filter_brand_id, #product_sell_report_form #variation_id, #product_sell_report_form #location_id, #product_sell_report_form #customer_id,#product_sell_report_form #ir_user_id'
  ).change(function ()
  {
    product_sell_report.ajax.reload();
    product_sell_grouped_report.ajax.reload();
  ...
  items_report_table = $('#items_report_table').DataTable({
    ...
    ajax: {
      ...
      data: function (d)
      {
        ...
        d.user_id = $('select#ir_user_id').val();
  ...
  $(document).on('change', '#ir_supplier_id, #ir_customer_id, #ir_location_id ,#ir_user_id', function ()
  {
    items_report_table.ajax.reload();
  });
...
function updateSalesRepresentativeReport()
{
  ...
  sr_instllment_report.ajax.reload();
  sr_sales_report.ajax.reload();
  support_tickets.ajax.reload();
  all_agent_table.ajax.reload();
  details_agent_table.ajax.reload();

  support_admin_tickets.ajax.reload();

  sr_sales_commission_report.ajax.reload();
  sr_sales_with_customer_groupcommission_report.ajax.reload();
...
function updateStatistics()
{
  ...
  var data_expense = {
    ...
    status: $('select#sr_sr_status').val(),
    start_date: start,
    end_date: end,
  };

  $('span#total_sell').html(__fa_awesome());
  $('span#net').html(__fa_awesome());
  $('span#invoice_due').html(__fa_awesome());
  $('span#total_sell_return').html(__fa_awesome());
  $('span#total_purchase').html(__fa_awesome());
  $('span#purchase_due').html(__fa_awesome());
  $('span#total_purchase_return').html(__fa_awesome());
  $('span#total_expense').html(__fa_awesome());

  $.ajax({
    method: 'GET',
    url: '/installment/dashboard',
    dataType: 'json',
    data: data_expense,
    success: function (data)
    {
      $('span#total_sell').html((data.totalDistinctContactIds));
      $('span#net').html((data.paidInstallments));
      $('span#invoice_due').html(__currency_trans_from_en(data.totalPaidInstallments, true));
      $('span#total_sell_return').html((data.unpaidInstallments));
      $('span#total_purchase').html(__currency_trans_from_en(data.roundedTotal, true));
      $('span#purchase_due').html((data.unpaidDueInstallmentsCount));
      $('span#total_purchase_return').html(__currency_trans_from_en(data.unpaidDueInstallmentsAmount, true));
      $('span#total_expense').html(__currency_trans_from_en(data.total, true));
    },
  });
}
...
$('select[name="sr_status"]').on('change', function ()
{
  var url = "/installment/data";
  var data = {
    sr_status: $(this).val(),
  };
  console.log(data);
  sr_instllment_report.ajax.url(url + "?" + $.param(data)).load();
});
$('select[name="sr_status"]').on('change', function ()
{
  var url = "/supportadmin/getdata";
  var data = {
    sr_status: $(this).val(),
  };
  console.log(data);
  support_admin_tickets.ajax.url(url + "?" + $.param(data)).load();
});
$('select[name="agent"]').on('change', function ()
{
  var url = "/agents/getdata";
  var data = {
    agent: $(this).val(),
  };
  console.log(data);
  all_agent_table.ajax.url(url + "?" + $.param(data)).load();
});
