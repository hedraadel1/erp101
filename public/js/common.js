...
$(document).ready(function ()
{
  ...
  // ============== show edit row modal div =====================
  $(document).on('click', "span.prod-name", function ()
  {
    var modl = $(this).parents('tr');
    modl.find('div.divmodal').appendTo('body');
  })
  ...
  $(document).on('click', '.input-number .quantity-up, .input-number .quantity-down', function ()
  {
    ...
    {
      input = tr.find('input.all');

    }
    else if ($(this).hasClass('part'))
    {//$(this).hasClass('part')
      input = tr.find('input.part');
      // input = $('input.part');
    }
...
$(document).on('click', '.sidebar-toggle2', function ()
{
  var sidebar_collapse = localStorage.getItem('upos_sidebar_collapse');
  if ($('body').hasClass('sidebar-collapse'))
  {
