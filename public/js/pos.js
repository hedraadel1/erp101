$(document).ready(function ()
{
  ...
  $('#variation_templates').on('change', function ()
  {
    var variation_templatt_id = $(this).val();
    var defaultOption = document.createElement('option');
    defaultOption.innerHTML = "يرجي الاختيرا";
    defaultOption.value = "all";
    if (variation_templatt_id)
    {
      $.ajax({
        url: "/products/get_variation_value/" + variation_templatt_id,
        type: "GET",
        dataType: "json",
        success: function (response)
        {
          $('select[name="variation_value_template"]').empty();
          $('select[name="variation_value_template"]').append(defaultOption);
          $.each(response.variation_value_templates, function (key, item)
          {
            $('select[name="variation_value_template"]').append('<option value="' +
  ...
  $('table#pos_table tbody').on('change', 'input.pos_quantity', function ()
  {
    ...
    $.ajax({
      ...
      {
        $.each(res.product_discount, function (key, item)
        {
          if (item.discount != null)
          {
            var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax')) - (item.discount_type == 'fixed' ? item.discount : ((__read_number(tr.find('input.pos_unit_price_inc_tax')) * item.discount) / 100));
            var line_total = entered_qty * unit_price_inc_tax;
  ...
  {
    $(this).parents('tr').addClass('remove_row_pos');
    ...
    $.ajax({
      ...
      method: 'GET',
      data: {
        'check_pass': 'remove_invoice_pos_item_pass',
      },
      success: function (result)
      {
        ...
        if (result == true)
        {
          ...
          $('#confirm_password').find('#page_type').val('remove_invoice_pos_item_pass');
          $('#confirm_password').find('#item_href').val('');
          $('#confirm_password').find('#item_href').val(row);
          $('#row').find('#password').val('');
          $('#confirm_password').find('button.confirm_password_pos').addClass('confirm_password_pos_item');
          $('#confirm_password').find('button.confirm_password_setting').addClass('confirm_password_pos_item');
          $('#confirm_password').find('div#msgErr').html('');
          $('#confirm_password').find('button.confirm_password_pos').removeClass('confirm_password_pos');
          $('#confirm_password').find('button.confirm_password_setting').removeClass('confirm_password_setting');
  ...
  $(document).on('click', 'button.confirm_password_pos_item', function (e)
  {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function (result)
      {
        ...
        if (result == 'true')
        {
          ...
          var row = $('tr.remove_row_pos');
          row.remove();
          pos_total_row();
        } else
        {
          $('.confirm_password_pos_item').parents().find('div#msgErr').html('');
          $('.confirm_password_pos_item').parents().find('div#msgErr').append('كلمة المرور غير صحيحة');
        }
      },
    });
  });
  ...
  get_product_suggestion_list(
    ...
    $('select#product_categorys2').val(),
    $('select#product_categorys3').val(),
    $('select#variation_value_template').val(),
  ...
  $('select#product_category,select#product_categorys2 ,select#product_categorys3,select#variation_value_template, select#product_brand, select#select_location_id').on('change', function (e)
  {
...
function get_product_suggestion_list(category_id, category2_id, category3_id, variation_value_template, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null)
{
  ...
  $.ajax({
    ...
    data: {
      ...
      variation_value_template: variation_value_template,
      brand_id: brand_id,
...
//Get invoice booking
function get_invoice_booking(element_obj)
{
  $.ajax({
    method: 'GET',
    url: '/sells/pos/get-invoice-booking',
    // data: { status: status, transaction_sub_type: transaction_sub_type },
    dataType: 'html',
    success: function (result)
    {
...
function get_product_booking(element_obj, search = null, contact_id = null)
{
  $.ajax({
    method: 'GET',
    url: '/sells/pos/get-product-booking',
    data: { search: search, contact_id: contact_id },
    dataType: 'html',
    success: function (result)
    {
        ...
        if (result.success)
        {
          ...
          if (result.enable_sr_no == '1')
          {
            var new_row = $('table#pos_table tbody')
              .find('tr')
              .last();
            let model = new_row.find('.row_edit_product_price_model').modal();
            $('form#add_pos_sell_form').append(model).css('display', 'block');
...
function calculate_balance_due()
{
  ...
  // -task-4
  __write_number($('#balance_due'), bal_due * 1);
}
/*
 task-4
 - on lcick check box #active_installment option 
*/
$(document).on('click', '#active_installment', function ()
{
  $('#show_installment').toggle(1000);
});
// =====================================================
...
$('div#product_list_body').on('scroll', function ()
{
  if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight)
  {
    ...
    var variation_value_template = $('select#variation_value_template').val();
    ...
    get_product_suggestion_list(category_id, variation_value_template, brand_id, location_id, null, is_enabled_stock, device_model_id);
  }
});
...
// get all invoice bookink
$(document).on('show.bs.modal', '#invoice_booking_modal', function ()
{
  get_invoice_booking($('div#invoice-booking-body'));
});
$('#search-product-name').on('keyup', function ()
{
  get_product_booking($('div#product-booking-body'), $(this).val());
});
$('#filter-customer_id').on('change', function ()
{
  get_product_booking($('div#product-booking-body'), null, $(this).val());
});

//get all product booking
$(document).on('show.bs.modal', '#product_booking_modal', function ()
{
  get_product_booking($('div#product-booking-body'));
});
...
$(document).on('click', '#send_for_sell_return', function (e)
{
  ...
  if (invoice_no)
  {
    $.ajax({
      ...
      method: 'GET',
      data: {
        'check_pass': 'enable_password_for_recovery',
      },
      success: function (result)
      {
        if (result == true)
        {
          $('#confirm_password').modal('show');
          $('#confirm_password').find('#page_type').val('enable_password_for_recovery');
          $('#confirm_password').find('#item_href').val('');
          $('#confirm_password').find('#item_href').val(invoice_no);
          $('#confirm_password').find('#password').val('');
          $('#confirm_password').find('div#msgErr').html('');
          $('#confirm_password').find('button.confirm_password_pos').addClass('confirm_password_pos_return');
          $('#confirm_password').find('button.confirm_password_pos').removeClass('confirm_password_pos');
...
$(document).on('click', 'button.confirm_password_pos_return', function (e)
{
  e.preventDefault();
  ...
  $.ajax({
    ...
    success: function (result)
    {
      ...
      if (result == 'true')
      {
        ...
        $('#confirm_password').modal('hide');
        var invoice_no = $('#confirm_password').find('#item_href').val();
        $.ajax({
          method: 'get',
          url: /validate-invoice-to-return/ + encodeURI(invoice_no),
          dataType: 'json',
      ...
      {
        $('.confirm_password_pos_return').parents().find('div#msgErr').html('');
        $('.confirm_password_pos_return').parents().find('div#msgErr').append('كلمة المرور غير صحيحة');
      }
    },
  });
});

$(document).on('click', '#send_for_sell_replace', function (e)
{
  var invoice_no = $('#send_for_sell_return_invoice_no').val();
  if (invoice_no)
  {
    $.ajax({
      ...
      method: 'GET',
      data: {
        'check_pass': 'enable_password_for_replace',
      },
      success: function (result)
      {
        // alert(result == true);
        ...
        {
          $('#confirm_password').modal('show');
          $('#confirm_password').find('#page_type').val('enable_password_for_replace');
          $('#confirm_password').find('#item_href').val('');
          $('#confirm_password').find('#item_href').val(invoice_no);
          $('#confirm_password').find('#password').val('');
          $('#confirm_password').find('div#msgErr').html('');
          $('#confirm_password').find('button.confirm_password_pos').addClass('confirm_password_pos_replace');
          $('#confirm_password').find('button.confirm_password_pos').removeClass('confirm_password_pos');
        ...
        } else
        {
          ...
          $.ajax({
            method: 'get',
            url: /validate-invoice-to-return-replace/ + encodeURI(invoice_no),
            dataType: 'json',
            success: function (result)
            {
              if (result.success == true)
...
$(document).on('click', 'button.confirm_password_pos_replace', function (e)
{
  e.preventDefault();
  ...
  $.ajax({
    ...
    success: function (result)
    {
      ...
      {
        $('.confirm_password_pos_replace').parents().find('div#msgErr').html('');
        $('.confirm_password_pos_replace').parents().find('div#msgErr').append('كلمة المرور غير صحيحة');
      }
    },
  });
});
