$(document).ready(function ()
{
  ...
  */
  $('.installment_option').on('click', function (event)
  {
    ...
    // var modal_date=tr.find('input.modal_istallment_date');
    // alert(modal_date.val())
    ...
    {
      $(document).on('change', '.modal_istallment_date', function ()
      {
        // var tr=$(this).parents('tr');
        // alert($(this).val())
        // alert($(this).value())
  ...
  home_installments_table = $('#home_installments_table').DataTable({
    processing: true,
    serverSide: true,
    ordering: false,
    searching: false,
    ...
    buttons: [],
    ajax: {
      "url": '/home/home-installments-table',
      "data": function (d)
      {
        if ($('#home-installments-table').length > 0)
        {
          d.location_id = $('#home-installments-table').val();
        }
      }
    },
    fnDrawCallback: function (oSettings)
    {
      __currency_convert_recursively($('#home_installments_table'));
    },
  });
