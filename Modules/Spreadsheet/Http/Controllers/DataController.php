...
{   
    ...
    public function modifyAdminMenu()
    {
        ...
        $is_product_enabled = is_product_enabled('267');
        $background_color = '';
        if (config('app.env') == 'demo') {
            $background_color = '#0086f9 !important';
        }
        if($is_product_enabled && !auth()->user()->can('superadmin')){
        //  $module_util->assignPermissions($this->user_permissions());
        }

        if (($is_spreadsheet_enabled ||  $is_product_enabled)&& auth()->user()->can('access.spreadsheet')) {
