...
class RepairController extends Controller
{
    ...
    public function index()
    {
        ...
        (($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') || $this->moduleUtil->hasThePermissionInSubscription($business_id, '229')) 
        && (auth()->user()->can('repair.view') || auth()->user()->can('repair.view_own')) ))) {
    ...
    public function editRepairStatus($repair_id)
    {
        ...
        if (!(auth()->user()->can('superadmin') ||
        (($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') || $this->moduleUtil->hasThePermissionInSubscription($business_id, '229')) 
          && auth()->user()->can('repair_status.update')))) {
            abort(403, 'Unauthorized action.');
        }
