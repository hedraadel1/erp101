...
@section('content')
    ...
    <section class="content">
        @component('components.widget', ['class' => 'box-solid'])
            @if (
                !empty($proposal_template) &&
                    auth()->user()->can('crm.access_proposal'))
                @slot('tool')
                    <div class="box-tools">
                        <a class="btn btn-primary pull-right m-5"
                            href="{{ action('\Modules\Crm\Http\Controllers\ProposalTemplateController@send') }}">
        ...
        $(document).ready(function() {
            proposals_table = $('#proposals').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'subject',
                        name: 'crm_proposals.subject'
                    },
                    {
                        data: 'sent_by_full_name',
                        name: 'sent_by_full_name'
                    },
                    {
                        data: 'created_at',
                        name: 'crm_proposals.created_at'
                    },
                    {
                        data: 'action',
                        name: 'action'
