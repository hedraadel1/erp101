...
class DataController extends Controller
{
    ...
    public function modifyAdminMenu()
    {
        ...
        $is_product_enabled =  is_product_enabled('220');
        if($is_product_enabled ){
         $module_util->assignPermissions($this->user_permissions());
        }
        $commonUtil = new Util();
        $is_admin = $commonUtil->is_admin(auth()->user(), $business_id);
        if (($is_crm_enabled || $is_product_enabled )&& auth()->user()->can('crm.view_dashboard')) {
            Menu::modify(
                'admin-sidebar-menu',
            ...
            if (!empty($crm_settings['enable_order_request'])) {
                ...
                $menu->whereTitle(__('sale.sale'), function ($sub) {
                    $sub->url(action('\Modules\Crm\Http\Controllers\OrderRequestController@listOrderRequests'),__('crm::lang.order_request'),['icon' => 'fa fas fa-sync', 'active' => request()->segment(2) == 'order-request']);
                });
            }
        }
    }
    ...
    */
    ...
    public function user_permissions()
    {
        $permissions = [
            [
              'value' => 'crm.view_dashboard',
              'label' => __('crm::lang.view_dashboard'),
              'default' => false
          ],
            [
