...
class LeadController extends Controller
{
    ...
    public function index()
    {
       ...
       ( $this->moduleUtil->hasThePermissionInSubscription($business_id, 'crm_module')) ||  $this->moduleUtil->hasThePermissionInSubscription($business_id, '220'))
        || !($can_access_all_leads || $can_access_own_leads)) {
            ...
            if ($lead_view == 'list_view') {
                return Datatables::of($leads)
                                    ...
                                    <button class="btn btn-info dropdown-toggle btn-xs" type="button"  data-toggle="dropdown" aria-expanded="false">
                                      ...
                                      <ul class="dropdown-menu dropdown-menu-left" role="menu">
                                       <li>
                                        ...
                                        <li>
                                            <a target="_blank" href="' . action('SellController@create', ['customer' => $row->id,'name'=>$row->name]) . '"class="">
                    ...
                    ->addColumn('last_follow_up', function($row) {
                        $html = '';
                      // dd($row->getCrmSchedules);
                        if (!empty($row->last_follow_up)) {
                          // dd($row->getCrmSchedules[0]->status);
                            $html .= isset($row->getCrmSchedules[0])?$row->getCrmSchedules[0]->title .'<br>':'--';
                            $html .= $this->commonUtil->format_date($row->last_follow_up, true) .'<br>';
                            $html .= isset($row->getCrmSchedules[0])?__('crm::lang.'.$row->getCrmSchedules[0]->status):'--'.'<br>';
                    ...
                    ->addColumn('follow_up_2', function($row) {
                        $html = '';
                      // dd($row->getCrmSchedules);
                        if (isset($row->getCrmSchedules[1])) {
                          $html .= isset($row->getCrmSchedules[1])?$row->getCrmSchedules[1]->title .'<br>':'--';
                          $html .= isset($row->getCrmSchedules[1])?$this->commonUtil->format_date($row->getCrmSchedules[1]->start_datetime, true).'<br>':'--';
                          $html .= isset($row->getCrmSchedules[1])?$row->getCrmSchedules[1]->status:'--'.'<br>';
                            $html .= '<br><a href="'.action('\Modules\Crm\Http\Controllers\ScheduleController@show', ['follow_up' => $row->getCrmSchedules[1]->id]).'" target="_blank" title="'.__("crm::lang.view_follow_up").'" data-toggle="tooltip">
                                <i class="fas fa-external-link-alt"></i>
                            </a><br>';
                        }
                    ...
                    ->addColumn('follow_up_3', function($row) {
                        $html = '';
                      // dd($row->getCrmSchedules);
                        if (isset($row->getCrmSchedules[2])) {
                            $html .= isset($row->getCrmSchedules[2])?$row->getCrmSchedules[2]->title .'<br>':'--';
                            $html .= isset($row->getCrmSchedules[2])?$this->commonUtil->format_date($row->getCrmSchedules[2]->start_datetime, true).'<br>':'--';
                            $html .= isset($row->getCrmSchedules[2])?$row->getCrmSchedules[2]->status :'--'.'<br>';
                            $html .= '<br><a href="'.action('\Modules\Crm\Http\Controllers\ScheduleController@show', ['follow_up' => $row->getCrmSchedules[2]->id]).'" target="_blank" title="'.__("crm::lang.view_follow_up").'" data-toggle="tooltip">
                                <i class="fas fa-external-link-alt"></i>
                            </a><br>';
                        }
                    ...
                    ->addColumn('new_schedule', function ($row) {
                      $statuses = Schedule::statusDropdown();
                      $follow_up_types = Schedule::followUpTypeDropdown();
                      $resource =$row;
                        return view('crm::lead.form-new_schedule',compact('statuses','follow_up_types','resource'));
                    })
                    ...
                    ->rawColumns(['action', 'crm_source', 'crm_life_stage','new_schedule', 'leadUsers', 'last_follow_up', 'follow_up_2', 'follow_up_3', 'upcoming_follow_up', 'created_at', 'name'])
                    ->make(true);
