...
class DataController extends Controller
{
    ...
    public function modifyAdminMenu()
    {
        ...
        if (auth()->user()->can('superadmin.access_package_subscriptions') && auth()->user()->can('business_settings.access')) {
            ...
            $menu->whereTitle(__('business.settings'), function ($sub) {
                $sub->url(
                    action('\Modules\Superadmin\Http\Controllers\SuperadminProductController@getBrandStore'),
                    __('superadmin::lang.brand_store'),
                    ['icon' => 'superadminimg', 'active' => request()->segment(1) == 'brand-store']
                );
            });
        }
        if (auth()->user()->can('superadmin.access_package_subscriptions') && auth()->user()->can('business_settings.access')) {
            ...
            $menu->whereTitle(__('business.settings'), function ($sub) {
                $sub->url(
                    action('\Modules\Superadmin\Http\Controllers\DepositRequestController@index'),
                    __('superadmin::lang.charging_wallet'),
                    ['icon' => 'superadminimg', 'active' => request()->segment(1) == 'brand-store']
                );
            });
        }
