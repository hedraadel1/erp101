...
class DataController extends Controller
{
    ...
    public function user_permissions()
    {
        return [
          [
            'value' => 'woocommerce.view_dashboard',
            'label' => __('woocommerce::lang.view_dashboard'),
            'default' => false
        ],
            [
    ...
    public function modifyAdminMenu()
    {
        ...
        $is_woo_enabled = (boolean)$module_util->hasThePermissionInSubscription($business_id, 'woocommerce_module', 'superadmin_package');
        $is_product_enabled =   is_product_enabled('221');
        if($is_product_enabled){
          $module_util->assignPermissions($this->user_permissions());
         }
        if (($is_woo_enabled ||$is_product_enabled)&& (auth()->user()->can('woocommerce.syc_categories') || auth()->user()->can('woocommerce.view_dashboard') ||auth()->user()->can('woocommerce.sync_products') || auth()->user()->can('woocommerce.sync_orders') || auth()->user()->can('woocommerce.map_tax_rates') || auth()->user()->can('woocommerce.access_woocommerce_api_settings'))) {
