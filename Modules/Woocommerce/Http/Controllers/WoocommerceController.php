...
class WoocommerceController extends Controller
{
    ...
    public function index()
    {
        try {
           ...
           ( $this->moduleUtil->hasThePermissionInSubscription($business_id, 'woocommerce_module')|| $this->moduleUtil->hasThePermissionInSubscription($business_id, '221')))) {
    ...
    public function mapTaxRates(Request $request)
    {
        ...
        $business_id = request()->session()->get('business.id');
        if (!(auth()->user()->can('superadmin') || 
        ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'woocommerce_module') 
