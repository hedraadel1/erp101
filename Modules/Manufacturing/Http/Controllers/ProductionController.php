...
class ProductionController extends Controller
{
    ...
    public function index()
    {
        ...
        ( $this->moduleUtil->hasThePermissionInSubscription($business_id, 'manufacturing_module')||$this->moduleUtil->hasThePermissionInSubscription($business_id, '215')))
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($productions)
                ->addColumn('action', function ($row) {
                    $html = '<button data-href="' .  action('\Modules\Manufacturing\Http\Controllers\ProductionController@show', $row->id) . '" class="btn btn-info btn-sm Btn-Brand btn-modal" data-container=".view_modal"><i class="fa fa-eye"></i> ' . __('messages.view') . '</button>';
                    if ($row->mfg_is_final == 0) {
                        $html .= ' <a href="' .  action('\Modules\Manufacturing\Http\Controllers\ProductionController@edit', $row->id) . '" class="btn Btn-Primary btn-sm Btn-Brand"><i class="fa fa-edit"></i> ' . __('messages.edit') . '</a>';

                        $html .= ' <button data-href="' . action('\Modules\Manufacturing\Http\Controllers\ProductionController@destroy', [$row->id]) . '" class="delete-production btn btn-sm Btn-Brand btn-danger"><i class="fa fa-trash"></i> ' . __("messages.delete") . '</button>';
                    }
