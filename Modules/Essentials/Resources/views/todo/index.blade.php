...
@section('content')
    ...
    <section class="content">
        ...
        @component('components.widget', [
            ...
            @if (is_product_enabled('265'))
                @slot('tool')
                    <div class="row">
                        <div class="col-md-12">
                            <div class="box-tools">
                                <button class="btn btn-block button-add"
                ...
                <table class="table table-bordered table-striped" id="task_table">
                    <thead>
                        <tr>
                            ...
                            <th> @lang('essentials::lang.task_type')</th>
    ...
    @include('essentials::todo.update_task_received_modal')
    {{-- <input type="hidden" id="check_task_url" value=""> --}}
@endsection
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change', "#priority_filter, #user_id_filter, #status_filter ,#project_id", function() {
                task_table.ajax.reload();
            });
        });
        ...
        $(document).on('click', '.change_is_received', function(e) {
            e.preventDefault();
            var task_id = $(this).data('task_id');
            var task = $(this).data('task');

            $('#update_task_received_modal').modal('show');
            $('#update_task_received_modal').find('#task_id').val(task_id);
            $('#update_task_received_modal').find('#task_name').html(task);
        });
