...
use Modules\Essentials\Notifications\AddNewCommentNotification;
use Modules\Essentials\Notifications\AddNewTaskNotification;
...
class ToDoController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    if (!(auth()->user()->can('superadmin') ||
      ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module') || $this->moduleUtil->hasThePermissionInSubscription($business_id, '216')))) {
    ...
    if (request()->ajax()) {
      ...
      if (!empty($request->project_id)) {
        $todos->where('project_id', $request->project_id);
      }
      ...
      return Datatables::of($todos)
        ->addColumn(
          ...
          function ($row) use ($is_admin, $auth_id) {
            ...
            if ($is_admin) {
              $html = '<li><a href="#" data-href="' . action('\Modules\Essentials\Http\Controllers\ToDoController@edit', [$row->id]) . '" class="btn-modal" data-container="#task_modal"><i class="glyphicon glyphicon-edit"></i> ' . __("messages.edit") . '</a></li>';
            }
            if ($is_admin) {
        ...
        ->editColumn('is_received', function ($row) {
          $html = '';
          $class = $row->is_received == 1 ? 'bg-green' : 'bg-red';
          $type =  $row->is_received == 1 ? 'تم الاستلام ' : ' لم يتم الاستلام ';
          if ($row->is_received == 1) {
            $html .= '<span class="label ' . $class . '"> ' . $type . '</span>';
          } else {
            $html .= '<a href="#" class="change_is_received" data-task="' . $row->task . '" data-task_id="' . $row->id . '"><span class="label ' . $class . '"> ' . $type . '</span></a>';
          }
        ...
        ->removeColumn('id')
        ->rawColumns(['task', 'action', 'status', 'is_received'])
        ->make(true);
    }
    ...
    $projects = Project::where('business_id', $business_id)->pluck('name', 'id')->toArray();
    return view('essentials::todo.index')->with(compact('projects', 'users', 'task_statuses', 'priorities'));
  }
  ...
  public function index_iframe(Request $request)
  {
    ...
    if (request()->ajax()) {
      ...
      return Datatables::of($todos)
        ...
        //     $html = '<span class="label ' . $class . '"> ' . $type . '</span>';
        //   } else {
        //     $html = '<a href="#" class="change_is_received" data_is_received="' . $row->is_received . '" data-task_id="' . $row->id . '"><span class="label ' . $class . '"> ' . $type . '</span></a>';
        //   }
        //     return  $html;
        // })
        ->editColumn('users', function ($row) {
  ...
  public function create()
  {
    ...
    return view('essentials::todo.create')->with(compact('projects', 'users', 'task_statuses', 'priorities'));
  }
  ...
  public function edit($id)
  {
    ...
    return view('essentials::todo.edit')->with(compact('projects', 'users', 'todo', 'task_statuses', 'priorities'));
  }
    ...
    if (request()->ajax()) {
      try {
        ...
        foreach ($users as $key => $value) {
          ...
          $url = action('\Modules\Essentials\Http\Controllers\ToDoController@show', $to_dos->id);
          $this->sendNotificationWhatsapp($value->contact_number, $message, $url);
        }
    ...
    if (request()->ajax()) {
      try {
        ...
        if ($request->has('only_status')) {
          ...
          $url = action('\Modules\Essentials\Http\Controllers\ToDoController@show', $todo->id);
          foreach ($todo->users as  $value) {
            $this->sendNotificationWhatsapp($value->contact_number, $message, $url);
          }
          $this->sendNotificationWhatsapp(optional($todo->assigned_by)->contact_number, $message, $url);
        }
        $output = [
          'success' => true,
          'msg' => __('lang_v1.success')
  ...
  public function addComment(Request $request)
  {
    ...
    if (request()->ajax()) {
      try {
        ...
        $message = " لقد قام  :" . $comment->added_by->getUserFullNameAttribute() .    '               بالرد علي التاسك ' . $comment->task->task;
        $url = action('\Modules\Essentials\Http\Controllers\ToDoController@show', $comment->task->id);
        foreach ($users as $key => $value) {
          if ($value->id != $auth_id) {
            $this->sendNotificationWhatsapp($value->contact_number, $message, $url);
          }
  ...
  public function uploadDocument(Request $request)
  {
    ...
    try {
      ...
      $message = " لقد قام  :" . auth()->user()->user_full_name .    '               بتحميل ملف جدبد  للتاسك ' . $todo->task;
      $url = action('\Modules\Essentials\Http\Controllers\ToDoController@show', $todo->id);
      foreach ($users as $key => $value) {
        if ($value->id != $auth_id) {
          $this->sendNotificationWhatsapp($value->contact_number, $message, $url);
  ...
  /**
   * change task to Received
   */
  public function changeReceived(Request $request)
  {
    $business_id = request()->session()->get('user.business_id');
    $todo = ToDo::where('business_id', $business_id)->where('id', $request->task_id)->first();
    $todo_before = $todo->is_received;
    $todo->update(['is_received' => 1]);
    ...
    $activity = Activity::forSubject($todo)
      ...
      ->first();
    $curl = curl_init();
    $message = " لقد قام  :" . optional($activity->causer)->user_full_name  .    '               باستلام التاسك ' . $todo->task;
    $url = action('\Modules\Essentials\Http\Controllers\ToDoController@show', $todo->id);
    $this->sendNotificationWhatsapp(optional($todo->assigned_by)->contact_number, $message, $url);

    $output = ['success' => true, 'msg' => 'تم استلام التاسك بنجاح'];
    return back()->with(['status' => $output]);
  }
  /**
   * check if user recevied task or no
   * send notification every 3h to user
   */
  public function cheackTaskReceivedAndDateLine(Request $request)
  {
    $business_id = request()->session()->get('user.business_id');
    if ($request->type == 'date_line') {
      $todos = ToDo::where('business_id', $business_id)->where('status', '!=', 'completed')->latest()->get();
    } else {
      $todos = ToDo::where('business_id', $business_id)->where('is_received', '0')->latest()->get();
    }

    foreach ($todos as $todo) {
      if ($request->type == 'date_line' && $todo->end_date <= date('Y-m-d H:i:s')) {
        $message = " التاسك بعنوان :" . $todo->task .    '          , قد تعدت الوقت المحدد ' . $todo->end_date . '  ,            بوقت  ' . $todo->estimated_hours .  '                          ساعة            من فضلك قم بالانتهاء من التاسك او اطلب من مديرك تعديل تاريخ انتهاء الديد لاين';
        ...
        $message = " لم  يتم استلام التاسك بعنوان  :" . $todo->task .    '               من فضلك قم بأستلام التاسك';
        foreach ($todo->users as  $value) {
          $url = action('\Modules\Essentials\Http\Controllers\ToDoController@show', $todo->id);
          $this->sendNotificationWhatsapp($value->contact_number, $message, $url);
        }
  ...
  private function sendNotificationWhatsapp($to, $message, $url)
  {
