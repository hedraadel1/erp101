...
class ShiftController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $input = $request->only(['name', 'type', 'holidays','deduction_from_salary']);
    ...
    public function getAssignUsers($shift_id)
    {
        ...
        if (auth()->user()->is_supervisor == 1) {
          $users = User::forDropdown($business_id, false,false , false,false , true);
        } else {
        ...
        return view('essentials::attendance.add_shift_users')
                ->with(compact('shift', 'users', 'user_shifts','user_shift_ids'));
    }
        ...
        try {
            ...
            foreach ($user_shifts as $key => $value) {
                if (!empty($value['is_added'])) {
                    ...
                    $this->set_user_shift_salary($shift_id , $key);
                }
            }
    ...
    public function set_user_shift_salary($shift_id , $user_id){
      $shift = Shift::where('id',$shift_id)->first();
        $start = Carbon::parse($shift->start_time);
        $end = Carbon::parse($shift->end_time);
        $duration = $end->diffInHours($start) ;
        $user = User::where('id',$user_id)->first();
        $shift_salary = $user->essentials_salary * $duration;
        $user->update(['shift_salary'=> $shift_salary]);
    }
}
