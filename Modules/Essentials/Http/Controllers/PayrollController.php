    ...
    public function index()
    {
        ...
        if (auth()->user()->can('essentials.create_payroll')) {
          if (auth()->user()->is_supervisor == 1) {
            $employees = $this->__getEmployeesByLocation($business_id , null , true);
          }else{
            $employees = $this->__getEmployeesByLocation($business_id , null , false);
          }
        }
  ...
  /   * @return Response
    ...
    public function create()
    {
        ...
        if (!empty($add_payroll_for)) {
            ...
            // to get start and end date of employee in shift
            $employee_shift = EssentialsUserShift::where('user_id',$add_payroll_for)->first();
            $payrolls = [];
            foreach ($employees as $employee) {
                ...
                $payrolls[$employee->id]['total_global_leaves'] = $this->essentialsUtil->getTotalLeavesForGivenDateOfAnBusinessLocation($business_id, $location_id, $start_date, $end_date);
                $payrolls[$employee->id]['total_days_holiday'] = $this->essentialsUtil->getTotalHolidayDaysOfAnEmployee($business_id, $employee_shift->essentials_shift_id, $employee_shift->start_date, $employee_shift->end_date);
                ...
                if ( $payrolls[$employee->id]['total_leaves'] >0) {
                        $payrolls[$employee->id]['leaves']['allowance_names'][] = 'اجازات  مدفوعه';
                        $payrolls[$employee->id]['leaves']['allowance_amounts'][] = $employee->shift_salary * $payrolls[$employee->id]['total_leaves'] ;
                        $payrolls[$employee->id]['leaves']['allowance_types'][] = 'fixed';
                        // $payrolls[$employee->id]['leaves']['allowance_percents'][] = $ad->amount_type == 'percent' ? $ad->amount : 0;
                }
                if ( $payrolls[$employee->id]['total_global_leaves'] >0) {
                        $payrolls[$employee->id]['global_leaves']['allowance_names'][] = 'اجازات  عامة مدفوعة';
                        $payrolls[$employee->id]['global_leaves']['allowance_amounts'][] = $employee->shift_salary * $payrolls[$employee->id]['total_global_leaves'] ;
                        $payrolls[$employee->id]['global_leaves']['allowance_types'][] = 'fixed';
                        // $payrolls[$employee->id]['leaves']['allowance_percents'][] = $ad->amount_type == 'percent' ? $ad->amount : 0;
                }
                if ( $payrolls[$employee->id]['total_days_holiday'] >0) {
                        $payrolls[$employee->id]['days_holiday']['allowance_names'][] = 'العطلات الاسبوعية ';
                        $payrolls[$employee->id]['days_holiday']['allowance_amounts'][] = $employee->shift_salary * $payrolls[$employee->id]['total_days_holiday'] ;
                        $payrolls[$employee->id]['days_holiday']['allowance_types'][] = 'fixed';
                        // $payrolls[$employee->id]['leaves']['allowance_percents'][] = $ad->amount_type == 'percent' ? $ad->amount : 0;
                }
            }
        ...
        try {
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                $transaction_ids[] = $transaction->id;
                 ...
                 // map transaction
                 $this->transactionUtil->map_to_chart_of_account('payroll', $transaction->id, false , 'is_salaries');
            ...
            foreach ($payments as $employee_id => $payment) {
                ...
                if ($transaction->payment_status != 'paid' && !empty($payment['final_total']) && !empty($payment['method'])) {
                    ...
                    DB::beginTransaction();
                        ...
                        //map payment
                        $this->transactionUtil->map_to_chart_of_account('payroll_payment', $tp->id ,true , 'is_salaries_paid');
                        $this->transactionUtil->auto_link_account($tp->id, $transaction->location_id, 'is_salaries');
    ...
    {   
        $business_id = request()->session()->get('user.business_id');
        if (!$this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')||$this->moduleUtil->hasThePermissionInSubscription($business_id, '216')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    private function __getEmployeesByLocation($business_id, $location_id = null , $check_supervisor = false)
    {
