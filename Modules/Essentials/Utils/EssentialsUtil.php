...
class EssentialsUtil extends Util
{
    ...
    public function checkUserShift($user_id, $settings, $clock_in_time = null)
    {
        ...
        $user_shifts = EssentialsUserShift::join('essentials_shifts as s', 's.id', '=', 'essentials_user_shifts.essentials_shift_id')
                    ...
                    ->select('essentials_user_shifts.*', 's.holidays', 's.start_time', 's.end_time', 's.type','s.deduction_from_salary')
                    ->get();
// dd($user_shifts);
    ...
    public function clockout($data, $essentials_settings)
    {
        ...
        $shift = Shift::where('id' ,$clock_in->essentials_shift_id )->first();
        $grace_after_checkin = !empty($settings['grace_after_checkin']) ? (int) $settings['grace_after_checkin'] : 0;
        ...
        if (!empty($clock_in)) {
            ...
            if ((! \Carbon::parse($clock_in->clock_in_time)->between($clock_in_start, $clock_in_end)) ) {
              $delay_time = \Carbon::parse($clock_in->clock_in_time)->diffInMinutes($clock_in_start);
              $user_essentials_salary = User::where('id',$data['user_id'])->first()->essentials_salary ?? 0;
              $amount = ($user_essentials_salary/60) * ($delay_time * $shift->deduction_from_salary );
              $employees = [$data['user_id']];
              // dd($amount);
              $this->storeAllowanceDeduction($amount , $employees , $delay_time);
            }
    ...
    {
        $leaves = EssentialsLeave::leftjoin('essentials_leave_types as lt','lt.id' ,'=' , 'essentials_leaves.essentials_leave_type_id')->where('essentials_leaves.business_id', $business_id)
                        ->where('lt.is_paid','1')
                        ->where('essentials_leaves.user_id', $employee_id)
                        ->whereDate('essentials_leaves.start_date', '>=', $start_date)
                        ->whereDate('essentials_leaves.end_date', '<=', $end_date)
                        ->get();
    ...
    public function getTotalLeavesForGivenDateOfAnBusinessLocation($business_id, $location_id, $start_date, $end_date)
    {
        $leaves = EssentialsHoliday::where('business_id', $business_id)
    ...
    public function getTotalHolidayDaysOfAnEmployee($business_id, $shift_id, $start_date, $end_date)
    {
        ...
        foreach ($shift as $key => $value) {
          $startDate = Carbon::parse($start_date)->next($value); // Get the first friday.
    ...
    public function getPayrollQuery($business_id)
    {
      ...
      if(auth()->user()->is_supervisor == 1) {
        $query->where('u.supervisor_id' , auth()->user()->id)
    ...
    public function storeAllowanceDeduction( $amount , $employees,$delay_time)
    {
        $business_id = request()->session()->get('user.business_id');
            $input = [];
            $input['description'] = ' د '.$delay_time .' خصم تاخير ';
            $input['type'] = 'deduction';
            $input['amount_type'] = 'fixed';
            $input['applicable_date'] = date('Y-m-d');
            $input['business_id'] = $business_id;

            $input['amount'] = $this->num_uf($amount);
            $allowance = EssentialsAllowanceAndDeduction::create($input);
            $allowance->employees()->sync($employees);
