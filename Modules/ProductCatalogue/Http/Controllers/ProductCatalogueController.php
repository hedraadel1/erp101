...
class ProductCatalogueController extends Controller
{
    ...
    public function index($business_id, $location_id)
    {
        ...
        // 
        $value = ['business_id' =>$business_id , 'location_id'=>$location_id];
        session(['catalog' => $value]);
        
        $sell_from_store = false;
        if ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'sell_from_store')) {
          $sell_from_store = true;
        }
        return view('productcatalogue::catalogue.index')->with(compact('sell_from_store','products', 'business', 'discounts', 'business_location', 'categories'));
    }
    ...
    public function generateQr()
    {
        ...
        if (!(auth()->user()->can('superadmin') ||
         ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'productcatalogue_module')|| is_product_enabled('217')))) {
